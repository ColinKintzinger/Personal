...
public:
    ...
    // iterator to ?rst entry. Remember: Inorder Traversal :) so the leftmost node in the tree
    Iterator begin();
...
typename LinkedBinarySearchTree<Elem>::Iterator LinkedBinarySearchTree<Elem>::finder(const Elem& k, const Iterator& itr) {
    ...
    if (const_cast<Elem&>(k) < itr.v->elt) return finder(k, itr.left()); // search left subtree
    else if (itr.v->elt < k) return finder(k, itr.right()); // search right subtree
    else return itr;     // found it here
    //if (itr.isExternal()) 
    //    return itr;   // value not found
    //if (k < itr.v->elt) 
    //    return finder(k, itr.left()); // search left subtree
    //else if (itr.v->elt < k) 
    //    return finder(k, itr.right()); // search right subtree
    //else 
       // return itr;     // found it here
}

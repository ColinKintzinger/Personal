// final_assingment_2023.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
void addNewbikeToCollection(Bicycle* newBicycle) {
    ...
    while (hashTable[hashedLocation] != nullptr) {
        cout << "Collision found in " << hashedLocation << endl;
        hashedLocation = (hashedLocation + 1) % 101; 
    }
...
Bicycle* findBike(Bicycle* newBicycle) {
    int hashedLocation = myHashFunction(newBicycle);
    while (hashTable[hashedLocation]->getbikeIdentificationNumber() != newBicycle->getbikeIdentificationNumber() ) 
        hashedLocation = (hashedLocation + 1) % 101;
